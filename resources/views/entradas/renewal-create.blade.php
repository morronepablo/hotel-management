...
@section('content_header')
    <h1><b>Registrar Renovación - Habitación {{ $entry->room->room_number }}</b></h1>
    <hr>
@stop
...
@section('content')
    <div class="row">
        <div class="col-md-12">
            ...
            <form action="{{ route('renewals.store') }}" method="POST" id="renewal-form">
                @csrf
                <input type="hidden" name="entry_id" value="{{ $entry->id }}">
                <input type="hidden" name="room_id" value="{{ $entry->room_id }}">
                <input type="hidden" name="room_type_id" value="{{ $entry->room_type_id }}">
                <input type="hidden" id="price_4_hours" value="{{ $entry->roomType->price_4_hours ?? 0 }}">
                <input type="hidden" id="price_full_night" value="{{ $entry->roomType->price_full_night ?? 0 }}">
                <input type="hidden" id="price_month" value="{{ $entry->roomType->price_month ?? 0 }}">

                <!-- 1. Card Habitación -->
                <div class="card card-outline card-primary">
                    <div class="card-header">
                        <h3 class="card-title">Habitación {{ $entry->room->room_number }}</h3>
                    </div>
                    <div class="card-body">
                        <div class="row">
                            <div class="col-md-4">
                                <strong>Precios:</strong><br>
                                4 Horas: $ {{ $entry->roomType->price_4_hours ?? 'N/A' }}<br>
                                Toda la Noche: $ {{ $entry->roomType->price_full_night ?? 'N/A' }}<br>
                                Mes: $ {{ $entry->roomType->price_month ?? 'N/A' }}
                            </div>
                            <div class="col-md-4">
                            ...
                            <div class="col-md-4">
                                ...
                                {{ $entry->roomType->description ?? 'N/A' }}
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div class="card card-outline card-primary">
                    ...
                    <div class="card-body">
                        ...
                        <div class="row">
                                    ...
                                    <select class="form-control select2 @error('client_id') is-invalid @enderror"
                                        ...
                                        @foreach ($clients as $client)
                                            <option value="{{ $client->id }}"
                                                {{ old('client_id', $entry->client_id) == $client->id ? 'selected' : '' }}>
                            ...
                            <div class="col-md-3">
                                <div class="form-group">
                                    <label for="check_in_date">Fecha de Inicio Renovación</label>
                                    <input type="date" class="form-control @error('check_in_date') is-invalid @enderror"
                                        name="check_in_date" id="check_in_date"
                                        value="{{ old('check_in_date', \Carbon\Carbon::parse($entry->check_out)->format('Y-m-d')) }}"
                                        required>
                                    @error('check_in_date')
                                        <span class="invalid-feedback" role="alert">{{ $message }}</span>
                                    @enderror
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-3">
                                <div class="form-group">
                                    <label for="check_in_time">Hora de Inicio</label>
                                    <input type="time" class="form-control @error('check_in_time') is-invalid @enderror"
                                        name="check_in_time" id="check_in_time"
                                        value="{{ old('check_in_time', \Carbon\Carbon::parse($entry->check_out)->format('H:i')) }}"
                                        required>
                                    @error('check_in_time')
                                        <span class="invalid-feedback" role="alert">{{ $message }}</span>
                                    @enderror
                            ...
                            <div class="col-md-3">
                                <div class="form-group">
                                    ...
                                    <input type="date"
                                        ...
                                        value="{{ old('check_out_date', \Carbon\Carbon::parse($entry->check_out)->format('Y-m-d')) }}"
                                        required>
                            ...
                            <div class="col-md-3">
                                <div class="form-group">
                                    ...
                                    <input type="time"
                                        ...
                                        value="{{ old('check_out_time', \Carbon\Carbon::parse($entry->check_out)->addHours(4)->format('H:i')) }}"
                                        required>
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control select2" id="modal_client_tipo_id" style="width: 100%" required>
                            ...
                            @foreach (\App\Models\TipoDocumento::orderBy('nombre', 'ASC')->get() as $tipo)
        ...
        $(document).ready(function() {
            ...
            function updatePricesAndDates() {
                try {
                    ...
                    // Obtener precios
                    const price4Hours = parseFloat($('#price_4_hours').val()) || 0;
                    const priceFullNight = parseFloat($('#price_full_night').val()) || 0;
                    const priceMonth = parseFloat($('#price_month').val()) || 0;
                    console.log('Precios:', {
                        price4Hours,
                        priceFullNight,
                        priceMonth
                    ...
                    console.log('Fecha y hora de inicio:', {
                        checkInDate,
                        checkInTime
                    });
    ...
    <div class="row">
        <div class="col-md-12">
            ...
            <form action="{{ route('renewals.store') }}" method="POST" id="renewal-form">
                ...
                <input type="hidden" name="room_id" value="{{ $entry->room_id }}">
                <input type="hidden" name="room_type_id" value="{{ $entry->room_type_id }}">
                <input type="hidden" id="selected_tariff_price" value="0">
                ...
                <div class="card card-outline card-primary">
                    ...
                    <div class="card-body">
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-3">
                                <div class="form-group">
                                    <label for="room_type_tariff_id">Tarifa / Precio</label>
                                    <select class="form-control select2 @error('room_type_tariff_id') is-invalid @enderror"
                                        name="room_type_tariff_id" id="room_type_tariff_id" required>
                                        <option value="">Seleccionar Tarifa</option>
                                        @foreach ($tariffs as $tariff)
                                            <option value="{{ $tariff->id }}"
                                                {{ old('room_type_tariff_id') == $tariff->id ? 'selected' : '' }}
                                                data-price="{{ $tariff->price }}">
                                                {{ $tariff->name }} - ${{ $tariff->price }}
                                            </option>
                                        @endforeach
                                    </select>
                                    @error('room_type_tariff_id')
                                        <span class="invalid-feedback" role="alert">{{ $message }}</span>
                                    @enderror
                                </div>
                            </div>
    ...
    <script>
        $(document).ready(function() {
            ...
            $('.select2').select2({
                placeholder: function() {
                    return $(this).attr('id') === 'client_id' ? 'Seleccionar Cliente' :
                        $(this).attr('id') === 'room_type_tariff_id' ? 'Seleccionar Tarifa' :
                        'Seleccionar';
                },
                allowClear: true,
                width: '100%'
            ...
            function updatePricesAndDates() {
                try {
                    ...
                    const tariffId = $('#room_type_tariff_id').val();
                    console.log('Tarifa seleccionada (ID):', tariffId);

                    const quantity = parseInt($('#quantity').val()) || 1;
                    console.log('Cantidad:', quantity);

                    // Obtener el precio de la tarifa seleccionada
                    let price = 0;
                    if (tariffId) {
                        const tariffOption = $('#room_type_tariff_id option[value="' + tariffId + '"]');
                        price = parseFloat(tariffOption.data('price')) || 0;
                        $('#selected_tariff_price').val(price);
                        console.log('Precio de la tarifa:', price);
                    } else {
                        $('#selected_tariff_price').val(0);
                        console.log('Sin tarifa seleccionada, precio establecido a 0');
                    }
                    ...
                    // Validar entradas
                    if (!checkInDate || !checkInTime || !tariffId) {
                        console.log('Faltan datos:', {
                            checkInDate,
                            checkInTime,
                            tariffId
                    ...
                    let checkOut;
                    // Ajustar check_out según la duración de la tarifa (esto requiere una columna 'duration' en room_type_tariffs)
                    checkOut = checkIn.clone().add(4 * quantity,
                    'hours'); // Por defecto 4 horas, ajusta según la tarifa
                    ...
                    if (checkOut && !isCheckOutModified) {
                        ...
                        console.log('checkOut no definido');
                    } else {
                        console.log('Check-out no actualizado (modificado manualmente)');
                    }

                    // Guardar el precio para updatePayment
                    $('#total').data('base-price', price * quantity);
                    updatePayment();
                } catch (error) {
                    console.error('Error en updatePricesAndDates:', error);
                }
            ...
            // Escuchar cambios para actualizar precios y fechas
            $('#room_type_tariff_id, #quantity, #check_in_date, #check_in_time').on('change', function() {
                console.log('Evento change disparado en:', this.id);
                updatePricesAndDates();
            });
            ...
            // Forzar actualización si room_type_tariff_id ya tiene un valor
            if ($('#room_type_tariff_id').val()) {
                console.log('Forzando actualización inicial de room_type_tariff_id...');
                $('#room_type_tariff_id').trigger('change');
            }
        ...
        <div class="col-md-12">
            ...
            <form action="{{ route('renewals.store') }}" method="POST" id="renewal-form">
                ...
                <div class="card card-outline card-primary">
                    ...
                    <div class="card-body">
                        @if ($entry->room && $entry->room->roomType)
                            <div class="row">
                                <div class="col-md-4">
                                    <strong>Precios:</strong><br>
                                    @if ($entry->room->roomType->roomTypeTariffs && $entry->room->roomType->roomTypeTariffs->count() > 0)
                                        @foreach ($entry->room->roomType->roomTypeTariffs as $tariff)
                                ...
                                <div class="col-md-4">
                                    <strong>Descripción:</strong><br>
                                    {{ $entry->room->roomType->description ?? 'N/A' }}
                                </div>
                            </div>
                        @else
                                ...
                                <div class="form-group">
                                    ...
                                    <select class="form-control select2 @error('room_type_tariff_id') is-invalid @enderror"
                                        ...
                                        @foreach ($tariffs as $tariff)
                                            <option value="{{ $tariff->id }}"
                                                {{ old('room_type_tariff_id') == $tariff->id ? 'selected' : '' }}
                                                data-price="{{ $tariff->price }}" data-type="{{ $tariff->type }}"
                                                data-duration="{{ $tariff->duration ?? 4 }}"
                                                data-hour-checkout="{{ $tariff->hour_checkout ?? '11:00' }}">
                                                {{ $tariff->name }} ({{ $tariff->type }} - $
        ...
        $(document).ready(function() {
            ...
            function updatePricesAndDates() {
                try {
                    ...
                    const selectedTariff = $('#room_type_tariff_id option:selected');
                    const tariffType = selectedTariff.data('type');
                    const price = parseFloat(selectedTariff.data('price')) || 0;
                    const duration = parseInt(selectedTariff.data('duration')) || 4; // Valor por defecto 4 horas
