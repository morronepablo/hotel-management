        ...
        <div class="col-md-6">
            <div class="card card-outline card-primary">
                <div class="card-header">
                    <h3 class="card-title my-1">Personal registrado</h3>
                    <div class="card-tools">
                        <a href="{{ route('acceso.personal.create') }}" class="btn btn-primary btn-sm"><i
                                class="fa fa-plus"></i>
                            Crear nuevo</a>
                    </div>
                </div>
                ...
                <div class="card-body">
                    <table id="personal-table" class="table table-striped table-bordered table-hover table-sm">
                                ...
                                <tr>
                                    ...
                                    <td class="text-center">
                                        <div class="btn-group" role="group" aria-label="Basic example">
                                            <a href="{{ route('acceso.personal.show', $staff->id) }}"
                                                class="btn btn-info btn-sm" data-bs-toggle="tooltip" data-bs-placement="top"
                                                title="Ver Personal">
                                                <i class="fas fa-eye"></i>
                                            </a>
                                            <a href="{{ route('acceso.personal.edit', $staff->id) }}"
                                                class="btn btn-success btn-sm" data-bs-toggle="tooltip"
                                                data-bs-placement="top" title="Editar Personal">
                                                <i class="fas fa-pencil"></i>
                                            </a>
                                            <form action="{{ route('acceso.personal.destroy', $staff->id) }}" method="POST"
                                                onclick="preguntar{{ $staff->id }}(event)"
                                                id="miFormulario{{ $staff->id }}">
                                                @csrf
                                                @method('DELETE')
                                            ...
                                            <script>
                                                function preguntar{{ $staff->id }}(event) {
                                                    ...
                                                    Swal.fire({
                                                        ...
                                                        if (result.isConfirmed) {
                                                            var form = $('#miFormulario{{ $staff->id }}');
                                                            form.submit();
                                                        }
                                                    });
                                                }
...
@section('css')
    <style>
        ...
        #personal-table_wrapper .dt-buttons {
            margin-bottom: 20px;
        }
...
@section('js')
    ...
    <script>
        $(document).ready(function() {
            ...
            table.buttons().container().appendTo('#personal-table_wrapper .col-md-12:eq(1)');
        });
